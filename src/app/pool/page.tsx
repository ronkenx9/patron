  ...
  return (
    <Shell>
      <section className="panel">
        ...
        <h2>Three pool actions.</h2>
        <p className="section-copy">
          This is the owner’s proof surface for the sprint: shield, private transfer, unshield. Only successful mainnet
          hashes touching the live pool belong in <code>strk20.json</code>.
        </p>
        <div className="actions">{(["shield", "tip", "unshield"] as Action[]).map((item) => <button className="pill" aria-current={action === item} key={item} onClick={() => setAction(item)}>{item}</button>)}</div>
        <div className="stack" style={{ marginTop: 20 }}>
          <FeeChip />
          <label>Amount (STRK)<input value={amount} onChange={(event) => setAmount(event.target.value)} inputMode="decimal" /></label>
          {action !== "shield" ? <label>{action === "tip" ? "Creator address" : "Withdrawal destination"}<input value={recipient} onChange={(event) => setRecipient(event.target.value)} placeholder="0x…" spellCheck={false} /></label> : <p className="warning">Shielding is public at the pool edge and your wallet will ask <b>twice</b> — first the ERC-20 approve, then the private deposit; that is expected, not a duplicate. Use dust only, and let notes mature ~10 blocks before a private tip.</p>}
          {!connected ? <ConnectButton /> : <button className="btn btn-solid" disabled={!strk20 || busy} onClick={run}>{busy ? "Proving…" : action}</button>}
